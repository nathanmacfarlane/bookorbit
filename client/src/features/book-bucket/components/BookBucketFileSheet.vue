...
import type { BookBucketFile, BookBucketMetadata, MetadataCandidate, MetadataSource, MetadataProviderKey } from '@projectx/types'
...
import { useBookBucketDetail } from '../composables/useBookBucketDetail'
...
const props = defineProps<{ file: BookBucketFile }>()

const emit = defineEmits<{
  close: []
  discarded: []
  updated: [BookBucketFile]
}>()

const { saved, saveError, saveMetadata, setTarget, discardFile, coverUrl } = useBookBucketDetail()
const { libraries, fetchLibraries: fetchLibs } = useLibraries()

const meta = computed(() => props.file.selectedMetadata ?? props.file.embeddedMetadata ?? ({} as BookBucketMetadata))
...
function buildMetadataPatchFromForm(): Partial<BookBucketMetadata> {
  return {
...
const backendBookBucketCoverUrl = computed(() => `${coverUrl(props.file.id)}?v=${new Date(props.file.updatedAt).getTime()}`)
const normalizedSelectedCoverUrl = computed(() => selectedCoverUrl.value.trim())
const currentBookBucketCoverUrl = computed(() => normalizedSelectedCoverUrl.value || backendBookBucketCoverUrl.value)
const currentBookBucketCoverFallbackUrl = computed(() => (normalizedSelectedCoverUrl.value ? backendBookBucketCoverUrl.value : null))

function onCurrentBookBucketCoverError(event: Event) {
  const img = event.target as HTMLImageElement
  const fallback = currentBookBucketCoverFallbackUrl.value
  if (!fallback) {
    ...
    >
      ...
      <div class="flex items-center gap-3 px-4 py-3 border-b border-border shrink-0 pr-12">
        <div class="relative size-12 rounded-lg bg-muted flex items-center justify-center shrink-0 overflow-hidden">
          <img
            :src="currentBookBucketCoverUrl"
            alt=""
            class="size-full object-cover"
            @load="($event.target as HTMLImageElement).style.display = ''"
            @error="onCurrentBookBucketCoverError"
          />
      ...
      <template v-else-if="metaView === 'diff' && selectedCandidate">
        <div class="flex-1 min-h-0">
          <MetadataDiffPanel
            ...
            :current-cover-url="currentBookBucketCoverUrl"
