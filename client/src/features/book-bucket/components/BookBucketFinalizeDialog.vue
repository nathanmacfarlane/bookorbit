...
import { useBookBucketFinalize } from '../composables/useBookBucketFinalize'
...
const { result, loading, error, finalize, reset } = useBookBucketFinalize()
...
async function fetchSelectionSummary() {
  try {
    const res = await api('/api/v1/book-bucket/files/selection-summary', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
