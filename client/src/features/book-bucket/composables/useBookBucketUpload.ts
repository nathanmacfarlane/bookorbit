...
function uploadSingle(item: UploadItem): Promise<void> {
  return new Promise((resolve) => {
    ...
    xhr.open('POST', '/api/v1/book-bucket/upload')
    ...
    xhr.onload = () => {
      if (xhr.status === 201) {
        ...
        item.progress = 100
        try {
          item.bookBucketFile = JSON.parse(xhr.responseText) as BookBucketFile
        } catch {
...
export function useBookBucketUpload() {
