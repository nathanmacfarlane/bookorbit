...
import type { StagingFile, StagingMetadata, MetadataCandidate, MetadataSource, MetadataProviderKey } from '@projectx/types'
...
import { useStagingDetail } from '../composables/useStagingDetail'
...
const props = defineProps<{ file: StagingFile }>()
...
const { saved, saveError, saveMetadata, setTarget, discardFile, coverUrl } = useStagingDetail()
const { libraries, fetchLibraries: fetchLibs } = useLibraries()

const meta = computed(() => props.file.selectedMetadata ?? props.file.embeddedMetadata ?? ({} as StagingMetadata))
