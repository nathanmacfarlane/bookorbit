...
import { useBookBucketFiles } from '@/features/book-bucket/composables/useBookBucketFiles'
import { useBookBucketSummary } from '@/features/book-bucket/composables/useBookBucketSummary'
import { useBookBucketStatistics } from '@/features/book-bucket/composables/useBookBucketStatistics'
import { useBookBucketUpload, SUPPORTED_FORMATS } from '@/features/book-bucket/composables/useBookBucketUpload'
import BookBucketToolbar from '@/features/book-bucket/components/BookBucketToolbar.vue'
import BookBucketFileList from '@/features/book-bucket/components/BookBucketFileList.vue'
import BookBucketFileSheet from '@/features/book-bucket/components/BookBucketFileSheet.vue'
import BookBucketFinalizeDialog from '@/features/book-bucket/components/BookBucketFinalizeDialog.vue'
import BookBucketBulkEditDialog from '@/features/book-bucket/components/BookBucketBulkEditDialog.vue'
import BookBucketSetDestinationDialog from '@/features/book-bucket/components/BookBucketSetDestinationDialog.vue'
...
const { summary, fetchSummary, subscribe, onBookBucketChange, socketConnected } = useBookBucketSummary()
const { statistics, fetchStatistics } = useBookBucketStatistics()
const { addFiles, isUploading } = useBookBucketUpload()

const selectedFile = ref<BookBucketFile | null>(null)
...
const unsubscribeChange = onBookBucketChange(() => {
...
function openSheet(file: BookBucketFile) {
...
function onFileUpdated(updated: BookBucketFile) {
...
const emptyMessage = computed(() => {
  ...
  return 'Upload files or drop them in the Book Bucket folder'
})

async function handleBulkDiscard() {
  const payload = getSelectionPayload()
  await api('/api/v1/book-bucket/files/discard', {
    method: 'POST',
...
async function handleApplyFetched() {
  const payload = getSelectionPayload()
  const res = await api('/api/v1/book-bucket/files/apply-fetched', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })
...
async function handleRetryFetch() {
  const payload = getSelectionPayload()
  const res = await api('/api/v1/book-bucket/files/retry-fetch', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })
...
async function fetchVisibleNamePreview() {
  ...
  try {
    const res = await api('/api/v1/book-bucket/files/preview-names', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
<template>
  <main class="flex-1" @dragover="onDragOver" @dragenter="onDragEnter" @dragleave="onDragLeave" @drop="onDrop">
    <div class="flex flex-col gap-4 py-4 sm:py-6 max-w-8xl w-full">
      <div class="flex items-center gap-2.5">
        ...
        <h1 class="text-xl font-semibold text-foreground tracking-tight">Book Bucket</h1>
        <span
      ...
      <div v-if="statistics && statistics.byFormat.length > 0" class="flex items-center gap-3 flex-wrap text-xs text-muted-foreground">
        <span class="font-medium text-foreground">{{ formatBytes(statistics.totalSizeBytes) }} in Book Bucket</span>
        <span
  ...
  <Teleport to="body">
    <BookBucketFileSheet v-if="selectedFile" :file="selectedFile" @close="closeSheet" @discarded="onDiscarded" @updated="onFileUpdated" />
  </Teleport>

  <BookBucketFinalizeDialog
  ...
  <BookBucketSetDestinationDialog
