...
@Injectable()
export class StagingIngestService {
  private readonly logger = new Logger(StagingIngestService.name);
  ...
  constructor(
    ...
    private readonly metadataService: StagingMetadataService,
  ...
  async ingestUpload(rawFilename: string, fileStream: Readable): Promise<number> {
    ...
    try {
      const destPath = await this.resolveUniquePath(join(this.stagingPath, filename));
  ...
  private extractMetadataAsync(fileId: number, absolutePath: string, format: string): void {
    const coversDir = join(this.stagingPath, 'covers');
    this.metadataService
      .extractAndSave(fileId, absolutePath, format, coversDir)
      .then(() => this.autoFetchMetadataAsync(fileId))
      .then(() => this.emitSummary())
      .then(() => this.events.emit(STAGING_FILE_INGESTED, fileId))
      .catch((err: Error) => this.logger.warn(`Background metadata extraction failed for staging file ${fileId}: ${err.message}`));
  }

  private async autoFetchMetadataAsync(fileId: number): Promise<void> {
    const enabled = await this.appSettings.isStagingAutoFetchEnabled();
    if (!enabled) return;
...
function computeConfidence(embedded: StagingMetadata, fetched: StagingMetadata): number {
