...
@WebSocketGateway({ namespace: '/staging', cors: { origin: process.env.CLIENT_URL ?? 'http://localhost:5173' } })
export class StagingGateway implements OnGatewayConnection, OnGatewayDisconnect {
  @WebSocketServer() server: Server;
  private readonly logger = new Logger(StagingGateway.name);
  ...
  emitSummary(summary: StagingSummary): void {
    this.server?.emit('staging:summary', summary);
  }
}
